...
-- Description: Complete T-SQL Schema script to recreate EstateNexusDBB
...
-- 1. Recreate Database
IF EXISTS (SELECT name FROM sys.databases WHERE name = N'EstateNexusDBB')
BEGIN
    ALTER DATABASE EstateNexusDBB SET SINGLE_USER WITH ROLLBACK IMMEDIATE;
    DROP DATABASE EstateNexusDBB;
END
GO
